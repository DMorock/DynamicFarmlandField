...
Shader "Custom/DFFCrossboardLOD2"
{
	Properties
	{
			...
			Pass
			{
				...
				CGPROGRAM
					...
					void geometryShader(point GS_INPUT p[1], inout TriangleStream<FS_INPUT> triStream)
					{
						...
						float2 size = p[0].parameters;
						//float halfSize = size.x;
						float4 tint;
						tint.xyz = p[0].color.xyz;
						tint.a = alpha;
						...
						// create billboard 1
						v[0] = float4(p[0].worldPosition + size.x * left, 1.0f);
						v[1] = float4(p[0].worldPosition + size.x * left + size.y * up, 1.0f);
						v[2] = float4(p[0].worldPosition - size.x * left, 1.0f);
						v[3] = float4(p[0].worldPosition - size.x * left + size.y * up, 1.0f);
